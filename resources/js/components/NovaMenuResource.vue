<template>
  <div class="py-3">
    <menu-builder-header :newMenuItem="newItemMenu" />
    <menu-builder
      v-model="menuItems"
    ...
    methods: {
      ...
      setMenuItemProperties(menuItems, localItemsState = null) {
        return menuItems.map(item => {
          const localItemState = Array.isArray(localItemsState) ? localItemsState.find(localItem => +localItem.id === +item.id) : false;
